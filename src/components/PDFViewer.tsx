...
export default function PDFViewer({ doc, onClose, onDelete }: Props) {
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-zinc-800 px-4 py-3">
        ...
        <button
          ...
          onClick={download}
          disabled={deleting}
          className="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition active:bg-blue-700 disabled:opacity-50"
        ...
        {onDelete && (
          <button
            ...
            disabled={deleting}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-rose-600/20 text-rose-300 transition active:bg-rose-600/35 disabled:opacity-50"
            aria-label="PDF'i sil"
            title="Sil"
          >
...
function Spinner() {
  return (
    <div className="h-4 w-4 animate-spin rounded-full border-2 border-rose-200/30 border-t-rose-200" />
  );
}
